...
void gpt2_forward(GPT2 *model, int *inputs, int *targets, int B, int T) {
  ...
  for (int mb = 0; mb < micro_batches_per_batch; mb++) {
    int mb_offset = mb * micro_batch_size;
    ...
    if (my_pe == 0) {
      ...
      for (int l = 0; l < 6; l++) {
        ...
        // Get layer activation pointers for this minibatch
        float *l_ln1 = acts.ln1 + l * B * T * C + mb_offset * T * C;
        float *l_ln1_mean = acts.ln1_mean + l * B * T + mb_offset * T;
        float *l_ln1_rstd = acts.ln1_rstd + l * B * T + mb_offset * T;
        float *l_qkvr = acts.qkvr + l * B * T * 3 * C + mb_offset * T * 3 * C;
        float *l_atty = acts.atty + l * B * T * C + mb_offset * T * C;
        float *l_att = acts.att + l * B * NH * T * T + mb_offset * NH * T * T;
        float *l_attproj = acts.attproj + l * B * T * C + mb_offset * T * C;
        float *l_residual2 = acts.residual2 + l * B * T * C + mb_offset * T * C;
        float *l_ln2 = acts.ln2 + l * B * T * C + mb_offset * T * C;
        float *l_ln2_mean = acts.ln2_mean + l * B * T + mb_offset * T;
        float *l_ln2_rstd = acts.ln2_rstd + l * B * T + mb_offset * T;
        float *l_fch = acts.fch + l * B * T * 4 * C + mb_offset * T * 4 * C;
        float *l_fch_gelu = acts.fch_gelu + l * B * T * 4 * C + mb_offset * T * 4 * C;
        float *l_fcproj = acts.fcproj + l * B * T * C + mb_offset * T * C;
        float *l_residual3 = acts.residual3 + l * B * T * C + mb_offset * T * C;
        float *scratch = acts.output + mb_offset * T * OUT_P;
