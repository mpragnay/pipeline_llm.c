...
void get_partition_for_pe(PipelinePartition* part, int my_pe, int n_pes, int total_layers) {
    int layers_per_pe = total_layers / n_pes;
    part->first_layer = my_pe * layers_per_pe;
    part->num_layers = layers_per_pe;
...
void gpt2_forward(GPT2 *model, int *inputs, int *targets, int B, int T) {
  ...
  // GPU 0: Embedding + first half of layers
  if (my_pe == 0) {
    ...
    nvshmem_putmem(model->nvshmem_act_buffer, last_output, B * T * C * sizeof(float), 1);
    nvshmem_quiet();
  ...
  // GPU 1: Second half of layers + final LN + loss
  if (my_pe == 1) {
    for (int l = 0; l < part.num_layers; l++) {
      if (l == 0) {
...
void gpt2_backward(GPT2 *model) {
  ...
  // GPU 1: Backward through classifier + second half layers
  if (my_pe == 1) {
    ...
    cudaCheck(cudaDeviceSynchronize());
    nvshmem_putmem(model->nvshmem_grad_buffer, dresidual, B * T * C * sizeof(float), 0);
    nvshmem_quiet();
  }

  nvshmem_barrier_all();

  // GPU 0: Backward through first half layers + embedding
  if (my_pe == 0) {
    ...
    for (int l = part.num_layers - 1; l >= 0; l--) {
      ...
      // Use scratch_btc for dl_btc since GPU 0 doesn't have lnf
      float *dl_btc = acts.scratch_btc;
      float *dl_bt4c = grads_acts.bt4c;
      float *dl_preatt = grads_acts.preatt;
      float *scratch = acts.output;
      float *buffer_a = l_atty;
...
int main(int argc, char *argv[]) {
  ...
  if (n_pes != 2) {
    if (my_pe == 0) {
      fprintf(stderr, "Error: This pipeline requires exactly 2 GPUs, got %d\n", n_pes);
    }
    nvshmem_finalize();
    MPI_Finalize();
    return 1;
  ...
  for (int step = 0; step <= train_num_batches; step++) {
    ...
    if ((step > 0 && step % sample_every == 0) || last_step) {
      ...
      for (int t = 1; t < genT; t++) {
        ...
        // Broadcast next token to all PEs
