...
except for those in the residual stream, where the gradients have to add. We
make sure that those parts work out ok and that we do a += as necessary. E.g.,
...
__global__ void softmax_forward_kernel5(float *out, float inv_temperature,
  ...
  int idx = (gridDim.x - blockIdx.x - 1) * warp.meta_group_size() +
            warp.meta_group_rank(); // backward order
  if (idx >= N * T) {
    return;
  }
  int own_pos = idx % T;
...
// this kernel performs a column-wise reduction over dout, in PyTorch equivalent
// to: dbias = dout.sum((0,1)) the idea is to employ one block to reduce along
// several columns, where each block has a width of 32 columns to ensure
// coalesced access. at the end we accumulate the reductions performed by the
// warps in each block via shared memory
__global__ void matmul_backward_bias_kernel4(float *dbias, const float *dout,
  ...
  const int warp_id =
      threadIdx.x / warpSize; // warp index in the block, 0,1,2,3
  const int lane_id =
      threadIdx.x % warpSize; // thread index in the warp, 0,1,2,...,31
  const int tl =
      blockIdx.x * warpSize; // pointer to the start column for this block
  ...
  // importantly, consecutive threads (in threadId) are processing adjacent
  // columns, leading to a coalesced memory access pattern
  float dout_sum = 0.0f;
...
// Implements linear interpolation using only two floating-point operations (as
// opposed to three in a naive implementation). Reference:
...
__device__ SoftmaxParams prepare_softmax_blockwide_nofloat4(
  ...
  // (shuffle) this results in much cleaner assembly than a multi-warp
  // cg::reduce
  ...
  // (warp-level) reduce sumval, thread 0 in each warp saves result in shared
  // memory
...
void matmul_backward(float *dinp, float *dweight, float *dbias, float *dout,
  ...
  if (dbias != NULL) {
    ...
    const int grid_size =
        OC / 32; // for now, OC must be divisible by 32 for this kernel to work
...
void attention_backward(float *dinp, float *dqkvr, float *dpreatt, float *datt,
  ...
  const float zero =
      0.0f; // note beta = 1.0f so that we accumulate gradients (+=)
...
// Backward pass is conceptually quite different from forward, because we can
// discard the activations of a layer as soon as we're done with it. This lets
// us aggressively reuse memory, so that we need far fewer tensors for backward
// state.
...
typedef struct {
  ...
  int micro_batch_size; // the micro-batch size for pipeline parallelism
  ...
  float *cpu_losses; // CPU buffer to copy the losses to, allocated with
                     // cudaMallocHost
  // NVSHMEM symmetric buffers for pipeline communication
...
void gpt2_allocate_nvshmem_buffers(GPT2 *model) {
  ...
  // Size for one layer's output: (B, T, C)
  ...
  if (my_pe == 0) {
    printf("allocated %zu MiB for NVSHMEM activation buffer\\n",
           model->nvshmem_buffer_size >> 20);
    printf("allocated %zu MiB for NVSHMEM gradient buffer\\n",
           model->nvshmem_buffer_size >> 20);
  }
...
void printer(int B, int T, int C, int micro_batch_no, float* arr, char* initial)
    {
      ...
      if (n > max_print) {
        printf("FORWARD[PE %d] micro_batch=%d ... (truncated, total %zu elements)\n", my_pe, micro_batch_no, n);
      }
...
void gpt2_forward(GPT2 *model, int *inputs, int *targets, int B, int T, bool log=false) {
  ...
  for (int mb = 0; mb < micro_batches_per_batch; mb++) {
    ...
    if(log) {
      printf("PE %d processing micro-batch %d/%d (batch_offset=%d, local_B=%d)\n",
            my_pe, mb, micro_batches_per_batch, batch_offset, local_B);
    }

    // GPU 0: Embedding + Layers 0-5 for this micro-batch
    if (my_pe == 0) {
      float *encoded = acts.encoded + (size_t)batch_offset * T * C;

      if(log) {
        // Debug: print encoded embeddings for this micro-batch
        printer(local_B, T, C, mb, encoded, "ENCODED ");
      }
      ...
      for (int l = 0; l < 6; l++) {
        ...
        // Get layer activation pointers (offset by batch_offset)
        ...
        float *scratch = acts.output + (size_t)batch_offset * T * OUT_P;

        // Forward pass through this layer (local_B)
        ...
        matmul_forward(l_fcproj, l_fch_gelu, l_fcprojw, l_fcprojb, local_B, T, 4 * C,
                      C);
        residual_forward(l_residual3, l_residual2, l_fcproj, local_B * T * C);
      }
      cudaCheck(cudaDeviceSynchronize());

      // Copy this micro-batch's layer 5 output to GPU 1's symmetric buffer directly
      size_t micro_batch_bytes = micro_batch_size * T * C * sizeof(float);
      float *layer5_output = acts.residual3 + (size_t)5 * B * T * C + (size_t)batch_offset * T * C;
      float *nvshmem_dest = model->nvshmem_act_buffer + (size_t)batch_offset * T * C;
      if (log) {
        printf("[PE %d] mb=%d src=%p dest=%p micro_batch_bytes=%zu total_output_bytes=%zu\n", my_pe, mb, (void*)layer5_output, (void*)nvshmem_dest, micro_batch_bytes, (size_t)B * T * C * sizeof(float));
        printer(local_B, T, C, mb, layer5_output, "FORWARD layer5_output ");
      }
      nvshmem_putmem(nvshmem_dest, layer5_output, micro_batch_bytes, 1);
      nvshmem_quiet();

      // GPU 0 doesn't compute loss, but needs to set mean_loss placeholder
      if (targets != NULL) {
        model->mean_loss = 0.0f; // placeholder for backward check
      } else {
        model->mean_loss = -1.0f; // inference mode
      }
    }

    // GPU 1: receive micro-batch slice and process layers 6-L, then final layernorm+loss
    nvshmem_barrier_all();

    if (my_pe == 1) {
      // copy received activations for this micro-batch
      size_t micro_batch_bytes = micro_batch_size * T * C * sizeof(float);
      float *nvshmem_src = model->nvshmem_act_buffer + (size_t)batch_offset * T * C;
      float *layer5_output = acts.residual3 + (size_t)5 * B * T * C + (size_t)batch_offset * T * C;
      if (log) {
        printf("[PE %d] mb=%d src=%p dest=%p micro_batch_bytes=%zu total_output_bytes=%zu\n", my_pe, mb, (void*)nvshmem_src, (void*)layer5_output, micro_batch_bytes, (size_t)B * T * C * sizeof(float));
        printer(local_B, T, C, mb, nvshmem_src, "FORWARD nvshmem_act_buffer ");
      }
      cudaMemcpy(layer5_output, nvshmem_src, micro_batch_bytes, cudaMemcpyDeviceToDevice);
      if (log) {
        printer(local_B, T, C, mb, layer5_output, "FORWARD layer5_output ");
      }

      // Process layers 6-11 for this micro-batch
      for (int l = 6; l < L; l++) {
      ...
      // Final layernorm and output (only on GPU 1) for this micro-batch
      residual = acts.residual3 + (size_t)(L - 1) * B * T * C + (size_t)batch_offset * T * C;
      ...
      float *outptr = acts.output + (size_t)batch_offset * T * OUT_P;
      matmul_forward(outptr, lnf, params.wte, NULL, local_B, T, C, Vp);

      // Compute loss if targets provided
      if (targets != NULL) {
        size_t tgt_elems = (size_t)local_B * T;
        ...
        // copy local losses to host and sum
  ...
  if (log) {
    printf("PE %d completed all micro-batches\n", my_pe);
  }

  // After processing all micro-batches process loss in PE 1
  if (my_pe == 1) {
    if (targets != NULL) {
      // DEBUG
      if (log) {
        ...
        for (int i = 0; i < max_print; ++i) {
          printf("[PE %d] cpu_loss[%d] = %f\n", my_pe, i, model->cpu_losses[i]);
        }
        if (n > max_print) {
      ...
      printf("Mean loss: %f computed on PE %d\n", mean_loss, my_pe);
    } else {
...
void gpt2_backward(GPT2 *model, bool log=false) {
  ...
  if (model->grads_memory == NULL) {
    ...
    // we're going to be clever for the activations backward pass. we don't need
    // to exactly mirror the forward pass acrtivations and we will save memory.
    size_t bw_act_sizes[NUM_ACTIVATION_TENSORS];
    GPT2Config cfg = model->config;
    cfg.num_layers =
        1; // copy the configuration but override number of layers to 1
  ...
  for (int mb = 0; mb < micro_batches_per_batch; mb++) {
    ...
    if (my_pe == 1) {
      ...
      matmul_backward(grads_acts.bt4c + (size_t)batch_offset * T * 4 * C, grads.wte, NULL, acts.output + (size_t)batch_offset * T * OUT_P, acts.lnf + (size_t)batch_offset * T * C,
                      params.wte, micro_batch_size, T, C, Vp);
      residual = acts.residual3 + (L - 1) * B * T * C + (size_t)batch_offset * T * C;
      layernorm_backward(dresidual + (size_t)batch_offset * T * C, grads.lnfw, grads.lnfb, grads_acts.bt4c + (size_t)batch_offset * T * 4 * C,
                        residual, params.lnfw, acts.lnf_mean + (size_t)batch_offset * T, acts.lnf_rstd + (size_t)batch_offset * T, micro_batch_size,
                        T, C);

      // Backward through layers 11-6
      for (int l = L - 1; l >= 6; l--) {
        residual = (l == 0) ? acts.encoded + batch_offset * T * C : acts.residual3 + (l - 1) * B * T * C + batch_offset * T * C;
        ...
        float *l_ln1 = acts.ln1 + l * B * T * C + batch_offset * T * C;
        float *l_ln1_mean = acts.ln1_mean + l * B * T + batch_offset * T;
        float *l_ln1_rstd = acts.ln1_rstd + l * B * T + batch_offset * T;
        float *l_qkvr = acts.qkvr + l * B * T * 3 * C + batch_offset * T * 3 * C;
        float *l_atty = acts.atty + l * B * T * C + batch_offset * T * C;
        float *l_att = acts.att + l * B * NH * T * T + batch_offset * T * T * NH;
        float *l_residual2 = acts.residual2 + l * B * T * C + batch_offset * T * C;
        float *l_ln2 = acts.ln2 + l * B * T * C + batch_offset * T * C;
        float *l_ln2_mean = acts.ln2_mean + l * B * T + batch_offset * T;
        float *l_ln2_rstd = acts.ln2_rstd + l * B * T + batch_offset * T;
        float *l_fch = acts.fch + l * B * T * 4 * C + batch_offset * T * 4 * C;
        float *l_fch_gelu = acts.fch_gelu + l * B * T * 4 * C + batch_offset * T * 4 * C;

        // Get gradient activation buffers
        float *dl_btc = acts.lnf + batch_offset * T * C;
        float *dl_bt4c = grads_acts.bt4c + batch_offset * T * 4 * C;
        float *dl_preatt = grads_acts.preatt + batch_offset * NH * T * T;
        float *scratch = acts.output + batch_offset * T * OUT_P;
        float *buffer_a = l_atty;
        float *buffer_b = l_fch;

        // Backward through this layer
        matmul_backward(dl_bt4c, dl_fcprojw, dl_fcprojb, dresidual + batch_offset * T * C, l_fch_gelu,
                        l_fcprojw, micro_batch_size, T, 4 * C, C);
        gelu_backward(dl_bt4c, l_fch, dl_bt4c, micro_batch_size * T * 4 * C);
        matmul_backward(dl_btc, dl_fcw, dl_fcb, dl_bt4c, l_ln2, l_fcw, micro_batch_size, T, C,
                        4 * C);
        layernorm_backward(dresidual + batch_offset * T * C, dl_ln2w, dl_ln2b, dl_btc, l_residual2,
                          l_ln2w, l_ln2_mean, l_ln2_rstd, micro_batch_size, T, C);
        matmul_backward(dl_btc, dl_attprojw, dl_attprojb, dresidual + batch_offset * T * C, l_atty,
                        l_attprojw, micro_batch_size, T, C, C);
        attention_backward(dl_bt4c, buffer_b, dl_preatt, scratch, buffer_a,
                          dl_btc, l_qkvr, l_att, micro_batch_size, T, C, NH);
        matmul_backward(dl_btc, dl_qkvw, dl_qkvb, dl_bt4c, l_ln1, l_qkvw, micro_batch_size, T, C,
                        3 * C);
        layernorm_backward(dresidual + batch_offset * T * C, dl_ln1w, dl_ln1b, dl_btc, residual, l_ln1w,
                          l_ln1_mean, l_ln1_rstd, micro_batch_size, T, C);
      }

      cudaCheck(cudaDeviceSynchronize());
      size_t micro_batch_bytes = micro_batch_size * T * C * sizeof(float);
      float *dresidual_output = dresidual + batch_offset * T * C;
      float *nvshmem_dest = model->nvshmem_grad_buffer + batch_offset * T * C;
      if (log) {
        printf("[PE %d] mb=%d src=%p dest=%p micro_batch_bytes=%zu total_output_bytes=%zu\n", my_pe, mb, (void*)dresidual_output, (void*)nvshmem_dest, micro_batch_bytes, (size_t)B * T * C * sizeof(float));
        printer(local_B, T, C, mb, dresidual_output, "FORWARD dresidual_output ");
      }
      // Transfer gradients to GPU 0 using nvshmem_putmem
      nvshmem_putmem(nvshmem_dest, dresidual_output,
                    micro_batch_size * T * C * sizeof(float), 0); // PE 0
      nvshmem_quiet();
    }
    ...
    if (my_pe == 0) {
      size_t micro_batch_bytes = micro_batch_size * T * C * sizeof(float);
      float* nvshmem_src = model->nvshmem_grad_buffer + batch_offset * T * C;
      float* dresidual_from_pe0 = dresidual + batch_offset * T * C;
      if (log) {
        printf("[PE %d] mb=%d src=%p dest=%p micro_batch_bytes=%zu total_output_bytes=%zu\n", my_pe, mb, (void*)nvshmem_src, (void*)dresidual_from_pe0, micro_batch_bytes, (size_t)B * T * C * sizeof(float));
        printer(local_B, T, C, mb, nvshmem_src, "BACKWARD nvshmem_grad_buffer ");
      }
      cudaMemcpy(dresidual_from_pe0, nvshmem_src, micro_batch_bytes,cudaMemcpyDeviceToDevice);
      if (log) {
        printer(local_B, T, C, mb, dresidual_from_pe0, "FORWARD dresidual_from_pe0");
      }
      ...
      for (int l = 5; l >= 0; l--) {
        ...
        float *l_qkvr = acts.qkvr + l * B * T * 3 * C + batch_offset * T * 3 * C;
        float *l_atty = acts.atty + l * B * T * C + batch_offset * T * C;
        float *l_att = acts.att + l * B * NH * T * T + batch_offset * NH * T * T;
        float *l_residual2 = acts.residual2 + l * B * T * C + batch_offset * T * C;
        float *l_ln2 = acts.ln2 + l * B * T * C + batch_offset * T * C;
        float *l_ln2_mean = acts.ln2_mean + l * B * T + batch_offset * T;
        float *l_ln2_rstd = acts.ln2_rstd + l * B * T + batch_offset * T;
      ...
      encoder_backward(grads.wte, grads.wpe, dresidual + batch_offset * T * C, model->inputs + batch_offset * T, micro_batch_size, T, C);
    }
  }
  // Synchronize before returning
  nvshmem_barrier_all();
...
int main(int argc, char *argv[]) {
  ...
  model.batch_size = B;
  model.seq_len = T;
  // configure micro-batching: default to no micro-batching (one micro-batch)
  if (micro_B == 0) {
    model.micro_batch_size = B;
  } else {
    if (B % micro_B != 0) {
      if (my_pe == 0) {
  ...
  printf("MICRO_BATCH_SIZE: %d, num micro batches: %d", model.micro_batch_size, model.micro_batches_per_batch);
